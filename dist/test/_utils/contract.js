...
exports.getTxContractEvents = getTxContractEvents;
exports.compressABI = compressABI;
exports.decompressABI = decompressABI;
...
function getTxContractEvents(web3, txReceipt, contractAddress, eventName, eventParams) {
    var events = [];
    if (txReceipt.events) for (var p in txReceipt.events) {
        events.push(txReceipt.events[p]);
    }return events.filter(function (log) {
        return log.address.toLowerCase() == contractAddress.toLowerCase();
    }).filter(function (log) {
        return log.raw.topics[0] == web3.utils.soliditySha3(eventName + "(" + eventParams.map(function (param) {
            return param.type;
        }).join(",") + ")");
    }).map(function (log) {
        return web3.eth.abi.decodeLog(eventParams.map(function (param) {
            var decodeParam = Object.assign({}, param);
