...
export default function runRewardsTests(web3: Web3, rp: RocketPool) {
  describe("Rewards", () => {
    ...
    before(async () => {
      ...
      // Create minipools
      let minipool1 = (await createMinipool(web3, rp, {
        from: registeredNode1,
        value: web3.utils.toWei("16", "ether"),
        gas: gasLimit,
      })) as MinipoolContract;
      let minipool2 = (await createMinipool(web3, rp, {
        from: registeredNode2,
        value: web3.utils.toWei("16", "ether"),
        gas: gasLimit,
      })) as MinipoolContract;
      let minipool3 = (await createMinipool(web3, rp, {
        from: registeredNode2,
        value: web3.utils.toWei("16", "ether"),
        gas: gasLimit,
      })) as MinipoolContract;
    ...
    it(printTitle("guardian", "set contract claimer percentage for rewards, then update it to zero"), async () => {
      ...
      let totalClaimersPerc = parseFloat(
        web3.utils.fromWei(
