...
export async function stake(
  ...
  // Get parameters
  let launchBalance = await rocketDAOProtocolSettingsMinipool.methods
    .getLaunchBalance()
    .call()
  ...
  let [details1, validatorMinipool1] = await Promise.all([getMinipoolDetails(), rp.minipool.getMinipoolByPubkey(validatorPubkey)]);
  ...
  let [details2, validatorMinipool2] = await Promise.all([getMinipoolDetails(), rp.minipool.getMinipoolByPubkey(validatorPubkey)]);
