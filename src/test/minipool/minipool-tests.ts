...
import {
    ...
    stakeMinipool, submitMinipoolWithdrawable
    ...
    describe('Minipool', () => {
        ...
        it(printTitle('random address', 'cannot send ETH to non-payable minipool delegate methods'), async () => {
            ...
            await shouldRevert(prelaunchMinipool.contract.methods.getStatus().send({
                ...
                gas: gasLimit,
            }), 'Sent ETH to a non-payable minipool delegate view method', 'Transaction reverted silently');
            ...
            await shouldRevert(refund(web3, rp, prelaunchMinipool, {
                ...
                value: web3.utils.toWei('1', 'ether'),
                gas: gasLimit
            }), 'Sent ETH to a non-payable minipool delegate mutator method', 'Transaction reverted silently');
        ...
        it(printTitle('node operator', 'cannot create a minipool if network capacity is reached and destroying a minipool reduces the capacity'), async () => {
            ...
            await setDAOProtocolBootstrapSetting(web3, rp, 'rocketDAOProtocolSettingsMinipool', 'minipool.maximum.count', minipoolCount, {from: owner, gas: gasLimit});
            // Creating minipool should fail now
            await shouldRevert(createMinipool(web3, rp, {from: node, value: web3.utils.toWei('32', 'ether'), gas: gasLimit}), 'Was able to create a minipool when capacity is reached', 'Minipool count after deposit exceeds limit based on node RPL stake');
            // Destroy a pool
            ...
            await createMinipool(web3, rp, {from: node, value: web3.utils.toWei('32', 'ether'), gas: gasLimit});
        });
        ...
        it(printTitle('node operator', 'can finalise a withdrawn minipool'), async () => {
            ...
            // Finalise
            await withdrawableMinipool.finalise({ from: nodeWithdrawalAddress, gas: gasLimit });
        ...
        it(printTitle('node operator', 'cannot finalise a withdrawn minipool twice'), async () => {
            ...
            await shouldRevert(withdrawableMinipool.finalise({ from: nodeWithdrawalAddress, gas: gasLimit }), "Was able to finalise pool twice", "Minipool has already been finalised");
        ...
        it(printTitle('node operator', 'cannot finalise a non-withdrawn minipool'), async () => {
            ...
            // Finalise
            await shouldRevert(withdrawableMinipool.finalise({ from: nodeWithdrawalAddress, gas: gasLimit }), 'Minipool was finalised before withdrawn', 'Minipool balance must have been distributed at least once');
        ...
        it(printTitle('random address', 'can slash node operator if withdrawal balance is less than 16 ETH'), async () => {
            ...
            await stakeMinipool(web3, rp, prelaunchMinipool, null, {from: node, gas: gasLimit});
            // Mark it as withdrawable
            await submitMinipoolWithdrawable(web3, rp, prelaunchMinipool.address, {from: trustedNode, gas: gasLimit});
            ...
            const balance = await rocketVault.methods.balanceOfToken('rocketAuctionManager', rocketTokenRPL.options.address).call().then((value: any) => web3.utils.toBN(value));
        ...
        it(printTitle('random', 'random address cannot withdraw and destroy a node operators minipool balance'), async () => {
            ...
            await shouldRevert(withdrawValidatorBalance(web3, rp, withdrawableMinipool, withdrawalBalance, random, true), 'Random address withdrew validator balance from a node operators minipool', "Invalid minipool owner");
