...
export async function withdraw(web3: Web3, rp: RocketPool, minipool: MinipoolContract, options: SendOptions) {
  ...
  let [nodeBalances1, minipoolBalances, nodeWithdrawn1] = await Promise.all([getNodeBalances(), getMinipoolBalances(), minipool.getNodeWithdrawn()]);
  ...
  let [nodeBalances2, nodeWithdrawn2] = await Promise.all([getNodeBalances(), minipool.getNodeWithdrawn()]);
