...
export default function runMinipoolScrubTests(web3: Web3, rp: RocketPool) {
	describe("Minipool Scrub Tests", () => {
		...
		it(printTitle("node", "cannot stake a prelaunch pool if scrub period has not elapsed"), async () => {
			await shouldRevert(stakeMinipool(web3, rp, prelaunchMinipool, {from: node, gas: gasLimit}), "Was able to stake minipool before scrub period elapsed", "Not enough time has passed to stake");
		});
		...
		it(printTitle("node", "cannot close a scrubbed minipool before funds are returned"), async () => {
			...
			await shouldRevert(close(web3, rp, prelaunchMinipool, { from: node, gas: gasLimit }), "Closed minipool before fund were returned", "Node ETH balance was not successfully transferred to node operator");
		});
		...
		it(printTitle("trusted node", "cannot vote to scrub twice"), async () => {
			await voteScrub(web3, rp, prelaunchMinipool, {from: trustedNode1, gas: gasLimit});
			await shouldRevert(voteScrub(web3, rp, prelaunchMinipool, {from: trustedNode1, gas: gasLimit}), "Was able to vote scrub twice from same member", "Member has already voted to scrub");
		});
		...
		it(printTitle("trust node", "cannot vote to scrub a staking minipool"), async () => {
			...
			await shouldRevert(voteScrub(web3, rp, prelaunchMinipool, {from: trustedNode1, gas: gasLimit}), "Was able to vote scrub a staking minipool", "The minipool can only be scrubbed while in prelaunch");
		});
		...
		it(printTitle("guardian", "can not set launch timeout lower than scrub period"), async () => {
			await shouldRevert(setDAOProtocolBootstrapSetting(web3, rp, "rocketDAOProtocolSettingsMinipool", "minipool.launch.timeout", scrubPeriod - 1, {from: owner, gas: gasLimit}), "Set launch timeout lower than scrub period", "Launch timeout must be greater than scrub period");
		});

		it(printTitle("guardian", "can not set scrub period higher than launch timeout"), async () => {
			await shouldRevert(setDAONodeTrustedBootstrapSetting(web3, rp, "rocketDAONodeTrustedSettingsMinipool", "minipool.scrub.period", launchTimeout + 1, {from: owner, gas: gasLimit}), "Set scrub period higher than launch timeout", "Scrub period must be less than launch timeout");
		});
	});
}
