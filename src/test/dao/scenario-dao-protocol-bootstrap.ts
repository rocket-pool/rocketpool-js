...
export async function setDAOProtocolBootstrapSettingMulti(web3: Web3, rp: RocketPool, _settingContractInstances: string[], _settingPaths: string[], _values: any[], options: SendOptions) {
    ...
    async function getTxData() {
        ...
        return Promise.all(instances.map((rocketDAOProtocolSettingsContract, index) => {
            switch (types[index]) {
                case 0:
                    return rocketDAOProtocolSettingsContract.methods.getSettingUint(_settingPaths[index]).call().then((value: any) => web3.utils.toBN(value));
                case 1:
