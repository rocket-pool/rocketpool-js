...
export async function placeBid(web3: Web3, rp: RocketPool, lotIndex: number, options: SendOptions) {
  ...
  let [lot1, balances1] = await Promise.all([getLotDetails(options.from), getBalances(options.from)]);
  ...
  let [lot2, balances2] = await Promise.all([getLotDetails(options.from), getBalances(options.from)]);
