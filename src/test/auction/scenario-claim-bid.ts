...
export async function claimBid(web3: Web3, rp: RocketPool, lotIndex: number, options: SendOptions) {
  ...
  let [details1, lot1, balances1] = await Promise.all([getContractDetails(), getLotDetails(options.from), getBalances(options.from)]);
  ...
  let [details2, lot2, balances2] = await Promise.all([getContractDetails(), getLotDetails(options.from), getBalances(options.from)]);
