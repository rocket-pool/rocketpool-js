...
export async function recoverUnclaimedRPL(web3: Web3, rp: RocketPool, lotIndex: number, options: SendOptions) {
	...
	function getLotDetails() {
		...
		]).then(([rplRecovered, remainingRplAmount]) => ({
			rplRecovered,
			remainingRplAmount,
		}));
	}
	...
	// Get initial details
	const [details1, lot1] = await Promise.all([getContractDetails(), getLotDetails()]);
	...
	// Get updated details
	const [details2, lot2] = await Promise.all([getContractDetails(), getLotDetails()]);
