...
export async function createLot(web3: Web3, rp: RocketPool, options: SendOptions) {
	// Get parameters
	const [lotMaxEthValue, lotDuration, startPriceRatio, reservePriceRatio, rplPrice] = await Promise.all([
	...
	function getContractDetails() {
		...
		]).then(([totalRplBalance, allottedRplBalance, remainingRplBalance, lotCount]) => ({
			totalRplBalance,
			allottedRplBalance,
			remainingRplBalance,
			lotCount: lotCount,
	...
	function getLotDetails(lotIndex: number) {
		...
		]).then(([exists, startBlock, endBlock, startPrice, reservePrice, totalRpl, currentPrice, claimedRpl, remainingRpl, isCleared]) => ({
			exists,
			startBlock,
			endBlock,
			startPrice,
	...
	const details1 = await getContractDetails();
	...
	const [details2, lot] = await Promise.all([getContractDetails(), getLotDetails(details1.lotCount)]);
