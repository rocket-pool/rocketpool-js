...
export async function mintDummyRPL(web3: Web3, rp: RocketPool, _account: string, _amount: string, options: SendOptions) {
    ...
    function getBalances() {
        return Promise.all([
            rocketTokenDummyRPL.methods.totalSupply().call(),
            rocketTokenDummyRPL.methods.balanceOf(_account).call(),
        ]).then(
            ([tokenSupply, userTokenBalance]) =>
                ({tokenSupply: web3.utils.toBN(tokenSupply), userTokenBalance: web3.utils.toBN(userTokenBalance)})
        );
    }
