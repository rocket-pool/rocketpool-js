...
export async function register(web3: Web3, rp: RocketPool, timezoneLocation: string, options: SendOptions) {
	...
	function getNodeDetails(nodeAddress: string) {
		return Promise.all([rp.node.getNodeExists(nodeAddress), rp.node.getNodeTimezoneLocation(nodeAddress)]).then(([exists, timezoneLocation]) => ({
			exists,
			timezoneLocation,
		}));
	}
	...
	const nodeCount1 = await rp.node.getNodeCount().then((value: any) => web3.utils.toBN(value));

	// Register
	await rp.node.registerNode(timezoneLocation, options);

	// Get updated node index & node details
	const nodeCount2 = await rp.node.getNodeCount().then((value: any) => web3.utils.toBN(value));

	const [lastNodeAddress, details] = await Promise.all([rp.node.getNodeAt(nodeCount2.sub(web3.utils.toBN(1)).toNumber()), getNodeDetails(options.from)]);
